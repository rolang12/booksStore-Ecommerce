...
class Reports extends Component
{
    ...
    public function SalesByDate()
    {
        ...
        //Si todo viene vacío entonces cortamos el flujo de información
        if ($this->reportType == 1 && ($this->dateFrom == '' || $this->dateTo == '' )) {
            return;
        }

        if ($this->userId == 0) {
            $this->data = Sale::join('users as u', 'u.id', 'user_id')
    ...
    public function getDetails($saleId)
    {
        ...
        ->select('sale_details.id', 'sale_details.price','sale_details.quantity',
        'p.name as product')->where('sale_details.sale_id', $saleId)->get();
